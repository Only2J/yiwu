...
import com.javaee.yiwu.until.Utils;
                ...
                if (result == SMSSDK.RESULT_COMPLETE){
                    ...
                    if (event == SMSSDK.EVENT_SUBMIT_VERIFICATION_CODE) {
                        ...
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(TeleLoginActivity.this,"登录成功",Toast.LENGTH_SHORT).show();
                                //如果需要跳转可在这里进行跳转页面
                                //登录成功跳转到软件首页页面
                                Intent intent=new Intent(TeleLoginActivity.this,MainActivity.class);
...
//                        });
                        ...
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(TeleLoginActivity.this,"验证码已发送",Toast.LENGTH_SHORT).show();
                            }
                        });
                    ...
                    try {
                        ...
                        if (!TextUtils.isEmpty(des)){
                            runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    Toast.makeText(TeleLoginActivity.this,des,Toast.LENGTH_SHORT).show();
                                }
                            });
                        }
